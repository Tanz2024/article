...
const countries = [
  ...
  'China', 'Denmark', 'Egypt', 'Finland', 'France', 'Germany', 'Greece', 'India', 'Indonesia', 'Italy', 
  'Japan', 'Mexico', 'Netherlands', 'Norway', 'Poland', 'Russia', 'Spain', 'Sweden', 'Switzerland', 'Turkey', 
...
export default function PersonalDetails() {
  ...
  const countryOptions = countries.map((name: string) => ({
    value: name,
    label: name
  }));
  ...
  const getUserAvatar = () => {
    if (details.avatar) {
      if (details.avatar.startsWith('/api/users/uploads/avatars/')) {
        return `http://localhost:5000${details.avatar}`;
      }
      return details.avatar;
    }
    return `https://ui-avatars.com/api/?name=${encodeURIComponent(getDisplayName())}&background=0ea5e9&color=ffffff&size=120`;
  };
        ...
        <div className="flex flex-col sm:flex-row items-center sm:items-start gap-4 sm:gap-6">
          <div className="relative shrink-0">
            <img
              src={getUserAvatar()}
              alt="Profile"
              className="w-20 h-20 sm:w-24 sm:h-24 rounded-full object-cover border-2 sm:border-4 border-white dark:border-slate-700 shadow-lg"
            />
            {editing && (
              <button
